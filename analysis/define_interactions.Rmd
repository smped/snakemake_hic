...
for (sz in bin_sizes) {
  ...
  a <- gi %>%
    ...
    labs(
      x = "Chromosome",
      y = glue("Total Significant Trans Interactions ({sz/1e3}kb bins)")
    ) +
    scale_y_continuous(expand = expansion(c(0, 0.05)), labels = comma)
  b <- gi %>%
    anchors() %>%
    ...
    labs(
      x = "Chromosome",
      y = glue("Significant Trans Interactions per Mb ({sz/1e3}kb bins)")
    ) +
    scale_y_continuous(expand = expansion(c(0, 0.05)))
  plot_grid(a, b, labels = c("A", "B"))
  ggsave(
  ...
  cat(
    glue("![*Trans interactions by chromosome using {sz/1e3}kb bins*](assets/interaction_summary_{sz}_trans.png)"),
    "\n"
  )
  cat("\n\n")
